...
def attack_blind_spot(player_id: int, blind_spot_id: int, answer: str, round_number: int) -> dict:
    with get_db_ctx() as db:
        ...
        # Verify answer: try to match against the original question in the DB
        user_answer = str(answer).strip()
        correct = False
        if spot["created_from_mistake_id"]:
            qrow = db.execute(
                "SELECT answer FROM questions WHERE content=? LIMIT 1",
                (round_row["question"],)
            ).fetchone()
            if qrow:
                correct = user_answer == str(qrow["answer"]).strip()
            else:
                # Question not in DB — fall back to trusting the user (self-grade)
                correct = True
        else:
            # No reference question — self-grade (assume correct)
            correct = True
