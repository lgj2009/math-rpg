...
def create_guild(name: str, description: str, player_id: int, username: str) -> dict:
    with get_db_ctx() as db:
        ...
        if existing:
            return {"detail": "Guild name already taken"}
        ...
        if in_guild:
            return {"detail": "You are already in a guild"}
...
def join_guild(guild_id: int, player_id: int, username: str) -> dict:
    with get_db_ctx() as db:
        in_guild = db.execute("SELECT guild_id FROM guild_members WHERE player_id=?", (player_id,)).fetchone()
        if in_guild:
            return {"detail": "Already in guild " + str(in_guild["guild_id"])}
...
def get_guild(guild_id: int, player_id: int = 0) -> dict:
    with get_db_ctx() as db:
        ...
        members = db.execute(
            "SELECT gm.*, p.username, p.level, p.title, p.xp FROM guild_members gm "
            "JOIN players p ON gm.player_id=p.id WHERE gm.guild_id=? ORDER BY gm.weekly_xp DESC",
            (guild_id,)).fetchall()
...
def attack_guild_boss(guild_id: int, player_id: int, username: str, damage: int) -> dict:
    with get_db_ctx() as db:
        ...
        # Log activity
        db.execute("INSERT INTO guild_activity (guild_id, player_id, username, action, detail, xp) "
                   "VALUES (?,?,?,?,?,?)",
                   (guild_id, player_id, username, 'boss_damage', f'Dealt {damage} damage', damage))
        if killed:
            db.execute("INSERT INTO guild_activity (guild_id, player_id, username, action, detail) "
                       "VALUES (?,?,?,?,?)",
        ...
        db.commit()
    return {"boss_hp": new_hp if not killed else new_max,
            "boss_max_hp": g["boss_max_hp"] if not killed else new_max,
